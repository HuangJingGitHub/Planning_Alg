#ifndef FEATURE_GEOMETRY_INCLUDED
#define FEATURE_GEOMETRY_INCLUDED
...
public:
    ...
    vector<float> DetermineTarget(vector<float> initial_pos, float desired_angle_val, vector<float> complete_pt_pos, 
                                  ...
                                  bool shape_constraint_active = true) {
        vector<float> target_pos(6, 0);
        if (desired_angle_val < 0 || complete_pt_idx < 0 || complete_pt_idx > 2 || complete_pt_pos.size() != 2 ||
            initial_pos.size() != 6 || shape_ratio_constraint < 0 || shape_ratio_constraint > 1) {
                cout << "Invalid input.\n";
                return target_pos;
            }
        vector<float> ref_pos(6, 0), dif_pos(2, 0);
        dif_pos[0] = complete_pt_pos[0] - initial_pos[0];
        dif_pos[1] = complete_pt_pos[1] - initial_pos[1];
        ...
        for (int i = 0; i <= 200; i++) {
            parameter_cnt = i / 2;
            cur_angle_parameter = (float)parameter_cnt / 100 * 2 * M_PI;
            if (CW_CCW)
                cur_angle_accumulated = cur_angle_parameter + desired_angle_val / 180 * M_PI;
            else
                cur_angle_accumulated = cur_angle_parameter - desired_angle_val / 180 * M_PI;
            CW_CCW = !CW_CCW;
            ...
            for (float ratio_1 = 1 - shape_ratio_constraint; ratio_1 <= 1 + shape_ratio_constraint; 
                ...
                ratio_2 += shape_ratio_constraint/5) {
                    ...
                    if (evaluation_H < min_evaluation_H) {
                        cout << min_evaluation_H << '\n';
                        min_evaluation_H = evaluation_H;
