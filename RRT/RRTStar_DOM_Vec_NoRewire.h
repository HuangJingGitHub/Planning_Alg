...
public:
    ...
    bool Plan(Mat source_img) {        
        ...
        while (CUR_GRAPH_SIZE < MAX_GRAPH_SIZE) {
            ...
            if (CUR_GRAPH_SIZE % 10 == 0) {
                rand_pos = target_pos_;
                CUR_GRAPH_SIZE++;
            }

            int nearest_idx = NearestNodeIdx(rand_pos, source_img);
            if (norm(rand_pos - graph_nodes_[nearest_idx]) < 10)
                continue;
            ...
            if (PathObstacleFree(graph_nodes_[nearest_idx], new_node)) {
                ...
                CUR_GRAPH_SIZE++;
                circle(source_img, new_node, 3, Scalar(0, 0, 0), -1);
    ...
    int NearestNodeIdx(Point2f rand_node, Mat source_img) {
        int res = 0;
       ...
       /*string str;
        if (graph_nodes_.size() % 500 == 0) {
            cin >> str;
            if (stoi(str) == 1) {
                line(source_img, rand_node, graph_nodes_[res], Scalar(0, 0, 255), 2);
            }
        }*/
